<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight dark:text-gray-200 leading-tight">
            {{ __('Daftar Pasien Menunggu Pemeriksaan') }}
        </h2>
    </x-slot>
                ...
                <div class="p-6 text-gray-900">
                    <div class="flex justify-between items-center mb-6">
                        ...
                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800">
                            {{ count($UnPeriksa) }} Pasien
                        </span>
                    </div>

                    @if($UnPeriksa->count() > 0)
                                    ...
                                    <tr class="hover:bg-gray-50">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                            <div class="flex flex-col">
                                                <span class="font-medium">{{ $item->jadwalPeriksa->hari }}</span>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                            <div class="flex justify-end space-x-2">
                                                <!-- Button Periksa -->
                                                <a href="{{ route('dokter.Memeriksa.create', $item->id) }}"
                                                   ...
                                                   onclick="console.log('Clicking ID: {{ $item->id }}', 'URL: {{ route('dokter.Memeriksa.create', $item->id) }}')">
                                                ...
                                                <!-- Button Tolak -->
                                                <form action="{{ route('dokter.Memeriksa.destroy', $item->id) }}" method="POST" class="inline"
                                                      onsubmit="return confirm('Apakah Anda yakin ingin menolak janji periksa ini?')">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit"
                    ...
                    @else
                        <div class="text-center py-12">
                            ...
                            <h3 class="mt-2 text-sm font-medium text-gray-900">Tidak ada pasien menunggu</h3>
                            <p class="mt-1 text-sm text-gray-500">
                                Belum ada pasien yang mendaftar untuk pemeriksaan hari ini.
                            </p>
                        </div>
                    @endif
                </div>
    ...
    <!-- Modal Konfirmasi (Optional) -->
    @push('scripts')
        <script>
            // Optional: Add any additional JavaScript for interactions
